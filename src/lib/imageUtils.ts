...
/**
 * Compress the image client-side, then upload it to Cloudflare R2 via our
 * /api/upload serverless function, returning a permanent public R2 URL
 * instead of a heavy inline Data URL. Falls back to the compressed Data URL
 * if the upload fails, so the admin flow never hard-blocks on a network hiccup.
 */
export async function processAndUploadImage(
  ...
  const dataUrl = await processImageForPlaceholder(file, placeholderType);
  ...
  try {
    ...
    console.warn('[R2 upload] falling back to data URL:', json?.error);
  } catch (err) {
    console.warn('[R2 upload] request failed, falling back to data URL:', err);
  }
...
/**
 * Kept as a no-op for backwards compatibility — previously flipped a flag when
 * Supabase's image transform endpoint failed. R2 URLs are served directly with
 * no transform step, so there's nothing to mark as failed; components still call
 * this on <img onError>, which is harmless.
 */
...
/**
 * Fallback raw URL extractor (strips any query-string transform params)
 */
...
/**
 * Optimizes an image URL for display (supports Cloudflare R2, Unsplash, Cloudinary, and Data URLs).
 * R2 public URLs are served as-is (already compressed client-side before upload — no
 * transform endpoint needed, which keeps this a single fast CDN hop).
 */
export function getOptimizedImageUrl(
  ...
  // Cloudflare R2 (VITE_R2_PUBLIC_URL) and any other plain CDN URL — already optimized at upload time
  return url;
}

/**
 * Generates a responsive srcset attribute for modern browsers.
 * R2-hosted images don't support on-the-fly resizing, so we only build a
 * srcset for services that do (Unsplash, Cloudinary); R2/plain URLs return ''.
 */
export function getResponsiveSrcSet(
  ...
  const isTransformable = url.includes('images.unsplash.com') || url.includes('res.cloudinary.com');
