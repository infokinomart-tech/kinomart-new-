...
import { isSupabaseConfigured, getSupabaseClient } from '../../lib/db';
...
export const AdminSettings: React.FC = () => {
  ...
  // Database Testing & Diagnostic State
      ...
      try {
        ...
        if (selectErr) {
          ...
          if (errStr.includes('does not exist') || errStr.includes('42p01') || errStr.includes('relation')) {
            res[t] = { status: 'missing', msg: 'টেবিল তৈরি করা হয়নি — neon_schema.sql রান করুন' };
          } else {
          ...
          if (writeErr) {
            res[t] = { status: 'error', msg: writeErr.message || 'Write failed' };
          } else {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSaveAll} className="space-y-6">
        ...
        {/* Section 6: Database & Media Storage Status */}
        <div className="bg-[#181B26] border border-[#2B3042] p-5 rounded-2xl space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-extrabold text-white text-sm flex items-center gap-2">
              <Database className="w-4 h-4 text-cyan-400" />
              ডাটাবেস ও মিডিয়া স্টোরেজ (Neon Postgres + Cloudflare R2)
            </h3>
            <span
            ...
            >
              ...
              {isSupabaseConfigured() ? 'কানেক্টেড (Connected)' : 'নট কানেক্টেড (Not Connected)'}
            </span>
          </div>

          <p className="text-xs text-[#94A3B8]">
            ডাটাবেস (Neon Postgres) এবং মিডিয়া স্টোরেজ (Cloudflare R2) সার্ভার সাইডে Vercel Environment Variables দিয়ে কনফিগার করা — এখানে কোনো ক্রেডেনশিয়াল দেওয়ার প্রয়োজন নেই। প্রজেক্ট প্রথমবার সেটআপ করতে <code className="text-cyan-300">neon_schema.sql</code> ফাইলটি আপনার Neon ডাটাবেসে একবার রান করুন।
          </p>
                ...
                {Object.values(testResults).some(r => r.status !== 'ok') && (
                  <div className="mt-3 p-3 bg-amber-500/10 border border-amber-500/30 rounded-xl text-amber-200 text-xs space-y-1">
                    ...
                    <p className="text-[11px] leading-relaxed">
                      Vercel Project Settings &gt; Environment Variables এ <strong>DATABASE_URL</strong> ঠিকভাবে সেট করা আছে কিনা যাচাই করুন, এবং <code className="text-amber-300">neon_schema.sql</code> আপনার Neon ডাটাবেসে রান করা হয়েছে কিনা নিশ্চিত করুন।
                    </p>
                  </div>
                )}
              </div>
