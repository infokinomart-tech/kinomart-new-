      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 bg-white border border-[#E8E3D9] rounded-3xl p-4 sm:p-8 shadow-xs">
        ...
        <div className="space-y-4">
          <div className="relative aspect-square w-full bg-[#FFDC33] rounded-2xl overflow-hidden border border-[#E8E3D9] shadow-inner group">
            ...
            <button
              ...
              className="absolute bottom-3 right-3 bg-black/75 hover:bg-black text-white text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5 backdrop-blur-xs transition-all shadow-md cursor-pointer"
            >
              <ZoomIn className="w-3.5 h-3.5 text-amber-300" />
              <span>ছবিটি বড় করে দেখুন</span>
            </button>
          </div>
        ...
        <div className="flex flex-col justify-between space-y-5">
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between gap-2 border-b border-[#E8E3D9] pb-3">
              <span className="text-xs font-bold text-[#6B7264] uppercase tracking-wider">
                ক্যাটাগরি: <span className="text-[#1F241E]">{product.category || 'গ্যাজেট'}</span>
              </span>
              <span className="bg-[#E2F0D9] text-[#2E5E1E] text-xs font-black px-3 py-1 rounded-full border border-[#C6E0B4]">
                ইন স্টক ({product.stock} টি উপলব্ধ)
              </span>
            </div>

            {/* Product Title */}
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-black text-[#1F241E] leading-tight">
              {product.name}
            </h1>
            ...
            <div className="flex items-center gap-2 text-xs text-[#6B7264]">
              ...
              <span className="text-gray-500 font-semibold">({product.reviewsCount || 10} রিভিউ)</span>
            </div>

            {/* Price Box */}
            <div className="bg-[#FAF8F5] rounded-2xl p-4 sm:p-5 border border-[#E8E3D9] flex items-baseline gap-3 shadow-2xs">
              <span className="text-2xl sm:text-3xl lg:text-4xl font-black text-[#5E6A45]">
            ...
            {variantList && variantList.length > 0 && (
              <div className="space-y-2 pt-1">
                <label className="text-xs sm:text-sm font-extrabold text-[#1F241E] block">
                  ফ্লেভার/কালার: <span className="text-[#5E6A45]">{selectedColor}</span>
                </label>
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-extrabold transition-all cursor-pointer ${
                        selectedColor === variant
                          ...
                          : 'bg-white border border-[#D5CEBF] text-[#1F241E] hover:bg-[#F2EFE8]'
                      }`}
                    >
                      {variant}
                    </button>
            ...
            <div className="flex items-center justify-between bg-white border border-[#E8E3D9] p-3 sm:p-4 rounded-2xl text-xs sm:text-sm font-bold text-[#1F241E]">
              <span>পরিমাণ (Quantity):</span>
              <div className="flex items-center gap-3">
                <div className="flex items-center border border-[#D5CEBF] rounded-xl overflow-hidden bg-[#FAF8F5] shadow-2xs">
                  <button
                    onClick={() => setQuantity(Math.max(1, quantity - 1))}
                    className="px-3 py-2 hover:bg-[#EFECE6] text-[#1F241E] transition-colors cursor-pointer"
                  >
                ...
                <span className="text-xs font-bold text-[#6B7264]">
                  অবশিষ্ট {product.stock} টি
                </span>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full bg-[#5E6A45] hover:bg-[#485333] active:scale-[0.98] text-white text-base sm:text-lg font-black py-4 px-6 rounded-2xl flex items-center justify-center gap-2.5 shadow-lg hover:shadow-xl transition-all cursor-pointer"
            >
              <Zap className="w-5 h-5 fill-white text-amber-300 animate-bounce" />
              <span>+ অর্ডার করতে চান</span>
            </button>

            {/* Trust Badges */}
            <div className="grid grid-cols-3 gap-2 pt-2 text-center">
              <div className="bg-[#FAF8F5] border border-[#E8E3D9] p-3 rounded-2xl flex flex-col items-center justify-center">
                <Truck className="w-5 h-5 text-[#5E6A45] mb-1" />
                <span className="text-[11px] font-bold text-[#1F241E]">
                  ক্যাশ অন ডেলিভারি
                </span>
              </div>
              <div className="bg-[#FAF8F5] border border-[#E8E3D9] p-3 rounded-2xl flex flex-col items-center justify-center">
                <ShieldCheck className="w-5 h-5 text-[#5E6A45] mb-1" />
                <span className="text-[11px] font-bold text-[#1F241E]">
                  ১০০% অরিজিনাল
                </span>
              </div>
              <div className="bg-[#FAF8F5] border border-[#E8E3D9] p-3 rounded-2xl flex flex-col items-center justify-center">
                <RotateCcw className="w-5 h-5 text-[#5E6A45] mb-1" />
                <span className="text-[11px] font-bold text-[#1F241E]">
                  ৭ দিনের রিটার্ন
                </span>
              </div>
